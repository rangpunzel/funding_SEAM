	          ...
	          <div class="card-tools">
	          	<table style="margin-left:10px;">
                	<tr>
						...
						<td style="padding-right:30px;">
							<div class="input-group row" style="width:400px;">
								...
								<input  class="form-control col-sm-8" type="text" name="keyword" placeholder="검색어를 입력하세요." value="${param.keyword }" style="width:180px;outline: 0;border-width: 0 0 2px;"/>
						...
						<td>
						<button type="button" id="downloadBtn" class="btn btn-block btn-secondary" style="float:right;width:200px;" onclick="fnExcelReport('hiddenTable','funseamding_일반회원리스트')">Excel로 다운로드 받기</button>
						</td>
					  </tr>	
					</table>
				</div>
				...
				<c:forEach items="${comMemberList }" var="comMember">
					<tr class="trForClick" style='font-size:0.85em;cursor:pointer;'>
						...
						<td><img src="<%= request.getContextPath() %>/displayProfileImg?mem_email=${comMember.mem_email}&fileName=${comMember.mem_profile_img}" onerror="this.src='<%= request.getContextPath() %>/resources/images/profile-picture.png'" alt="user profile image" style="width:100px;height:100px;"></td>
						<td>${comMember.mem_phone }</td>
						<td>
							<fmt:formatDate value="${comMember.mem_regdate }" pattern="yyyy-MM-dd"/>
						</td>
						<td onclick="event.cancelBubble=true"><button type="button" class="btn" onclick="OpenWindow('<%= request.getContextPath() %>/sendSMSForm?mem_name=${comMember.mem_name }&mem_phone=${comMember.mem_phone }','문자 보내기','700','735')"><i class="far fa-comment-dots fa-3x"></i></button></td>
					<td onclick="event.cancelBubble=true"><button type="button" class="btn" onclick="OpenWindow('<%= request.getContextPath() %>/sendEmailForm?mem_name=${comMember.mem_name }&mem_email=${comMember.mem_email }','메일 보내기','700','800')"><i class="far fa-envelope fa-3x"></i></button></td>
					</tr>
				</c:forEach>
	          </tbody>
	         </table>
