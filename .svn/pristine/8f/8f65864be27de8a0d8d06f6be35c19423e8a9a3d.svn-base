          ...
          <div class="card-body table-responsive p-0" style="height: auto;">
          	<div class="card-tools">
          		<table style="margin-left:50px;">
          			<tr>
						...
						<td>
							<button type="button" id="downloadBtn" class="btn btn-block btn-secondary float-right" style="float:right;width:200px;" onclick="fnExcelReport('hiddenTable','funseamding_일반회원리스트')">Excel로 다운로드 받기</button>
						</td>
					</tr>
				</table>
            </div>
			...
			<c:forEach items="${memberList }" var="member">
				<tr class="trForClick" style='font-size:0.85em;cursor:pointer;'>
					...
					<td><img src="<%= request.getContextPath() %>/displayProfileImg?mem_email=${member.mem_email}&fileName=${member.mem_profile_img}" onerror="this.src='<%= request.getContextPath() %>/resources/images/profile-picture.png'" alt="user profile image" style="width:100px;height:100px;"></td>
					<td>${member.mem_phone }</td>
					<td>
						<fmt:formatDate value="${member.mem_regdate }" pattern="yyyy-MM-dd"/>
					</td>
					<td onclick="event.cancelBubble=true"><button type="button" class="btn" onclick="OpenWindow('<%= request.getContextPath() %>/sendSMSForm?mem_name=${member.mem_name }&mem_phone=${member.mem_phone }','문자 보내기','700','735')"><i class="far fa-comment-dots fa-3x"></i></button></td>
					<td onclick="event.cancelBubble=true"><button type="button" class="btn" onclick="OpenWindow('<%= request.getContextPath() %>/sendEmailForm?mem_name=${member.mem_name }&mem_email=${member.mem_email }','메일 보내기','700','800')"><i class="far fa-envelope fa-3x"></i></button></td>
				</tr>
			</c:forEach>
          </tbody>
         </table>
